...
/**
 @brief Construct a motor contoller using the supplied led.
 
 @param ledp - pointer to the led object to be used for the odometer.
 
 */
Motor::Motor():
//rtos::Semaphore(),
bemfSensor(MOTOR_BEMF,BEMF_CHNL),
...
/**
 ...
 It initialises the pwm hardware, the BEMF sensor and odometer (if present).  It sets the new reading
 callback routing.
...
/*
 @brief Issue motor stopped report
 
 This  reports that the motor has stopped.  It overrides a virtual routine in the odometer class.
 
 @param trip - the current trip counter reading.
 
void Motor::_reportStopped(int trip)
{
    queueReport(LOCO_STOP, trip);
}
 */
